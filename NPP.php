...
$DOI = "";
if ($UpId != "") {
    $findSql = "SELECT * FROM npaperpublication WHERE npaperpublication_id='$UpId'";
...
if ($_SERVER['REQUEST_METHOD']   == 'POST') {
 ...
 ,`npaperpublication_user_id`)
  VALUES('$Author1','$Author2','$PaperTitle','$JournalName','$DOP','$Volume','$Pagenos','$DOI','$user_id','$user_id');";
        ...
        $sql = "UPDATE `npaperpublication` SET `Author1` = '$Author1', `Author2` = '$Author2', `PaperTitle` = '$PaperTitle', `JournalName` = '$JournalName', `DOP` = '$DOP', `Volume` = '$Volume', `Pagenos` = '$Pagenos', `DOI` = '$DOI' WHERE `npaperpublication`.`npaperpublication_id` = '$UpId';";
        if ($mysqli->query($sql) == true) {
...
<head>
    ...
    <title>National Level Paper Publication</title>
