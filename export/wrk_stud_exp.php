...
if ($query->num_rows > 0) {
    ...
    while ($row = $query->fetch_assoc()) {
        $lineData = array($row['wsorgfstud_id'],$row['workshoporg'],$row['workshopname'], $row['fromdate'], $row['todate'], $row['level'], get_Added_Name($row['wsorgfstud_user_id']), get_Added_Name($row['wsorgfstud_added_by']));
